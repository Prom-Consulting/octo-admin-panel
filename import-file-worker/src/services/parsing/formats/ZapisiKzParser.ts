...
import type { AssignmentStatus } from "../../../../../src/modules/assignments/models/Assignment.ts";

export class ZapisiKzParser extends ExcelParser<ZapisiKzRecord> {
  private mapStatus(status: AssignmentStatus): string {
  ...
  async parseFile(filePath: string): Promise<ParsedZapisiKzResult> {
    ...
    const result = await this.parseExcelFile(filePath, "zapisi_kz", (row, rowIndex) => {
  ...
  protected override getStartRowIndex(sheet: ExcelJS.Worksheet): number {
    return 3;
  }
}